...
async function openBoothPage(page: import('@playwright/test').Page): Promise<void> {
  ...
  await stubMarketDetail(page, createMarketDetail({
    dailyAvailability: [{ applyDate: '2026-08-01', totalStalls: 2, remainingStalls: 1 }],
  }));
  ...
  await expect(page.locator('#market-booth-A01')).toHaveClass(/booth-available/, {
    timeout: 15_000,
  });
}
...
test.describe('攤主互動式選位', () => {
  test('@smoke VENDOR-BOOTH-02／04 地圖區分可選及已占用攤位', async ({ page }) => {
    ...
    const available = page.locator('#market-booth-A01');
    const occupied = page.locator('#market-booth-A02');
  ...
  test('@smoke VENDOR-BOOTH-05 攤位被搶先選走時提示並引導重新選擇', async ({ page }) => {
    await openBoothPage(page);
    await page.locator('#market-booth-A01').click();
